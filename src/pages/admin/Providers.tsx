  ...
  return (
    <div className="space-y-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl md:text-5xl font-display font-black text-white tracking-tighter mb-2">Providers</h1>
          <p className="text-gray-500 font-black text-[10px] uppercase tracking-[0.2em]">Connect and manage external SMM APIs</p>
        </div>
        <button 
          onClick={() => {
            setEditingProvider(null);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {providers.map((provider, idx) => (
          <motion.div
            ...
            className="bg-brand-card p-8 rounded-[3rem] shadow-2xl border border-white/5 relative overflow-hidden group"
          >
            <div className="flex items-center justify-between mb-8">
              <div className="w-16 h-16 bg-brand-blue/10 rounded-2xl flex items-center justify-center text-brand-blue text-2xl group-hover:scale-110 transition-transform border border-brand-blue/10">
                <FontAwesomeIcon icon={faPlug} />
              </div>
              <span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border ${
                provider.status === 'Active' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-white/5 text-gray-600 border-white/5'
              }`}>
                {provider.status}
              </span>
            </div>
            ...
            <div className="space-y-4 mb-8">
              <h3 className="text-2xl font-display font-black text-white tracking-tighter">{provider.name}</h3>
              <div className="text-[10px] font-mono text-gray-500 truncate bg-white/5 p-4 rounded-2xl border border-white/5">{provider.url}</div>
              <div className="flex items-center justify-between pt-4 border-t border-white/5">
                <span className="text-[10px] font-black text-gray-600 uppercase tracking-widest">API Status</span>
            ...
            <div className="grid grid-cols-3 gap-3">
              <button className="p-4 rounded-2xl bg-white/5 text-gray-500 hover:text-brand-purple hover:bg-brand-purple/10 transition-all flex items-center justify-center border border-white/5">
              ...
              <button 
                ...
                className="p-4 rounded-2xl bg-white/5 text-gray-500 hover:text-brand-blue hover:bg-brand-blue/10 transition-all flex items-center justify-center border border-white/5"
              >
                <FontAwesomeIcon icon={faEdit} />
              </button>
              <button 
                onClick={() => handleDelete(provider.id)}
                className="p-4 rounded-2xl bg-white/5 text-gray-500 hover:text-rose-500 hover:bg-rose-500/10 transition-all flex items-center justify-center border border-white/5"
              >
        ...
        {providers.length === 0 && !isLoading && (
          <div className="col-span-full py-32 text-center space-y-4">
            ...
            <div className="text-gray-600 font-black uppercase tracking-widest text-xs">No API providers connected</div>
          </div>
        )}
      </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <motion.div 
            ...
            className="bg-brand-card w-full max-w-2xl rounded-[3rem] p-8 md:p-12 shadow-2xl border border-white/10 relative z-10"
          >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  ...
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none text-gray-600">
                    ...
                    <input
                      ...
                      className="w-full pl-12 pr-5 py-5 bg-white/5 border border-white/10 rounded-2xl text-white focus:outline-none focus:ring-4 focus:ring-brand-purple/10 focus:border-brand-purple transition-all font-bold"
                      placeholder="••••••••••••••••"
                    />
                  </div>
                </div>
