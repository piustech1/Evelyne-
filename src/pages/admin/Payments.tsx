  ...
  return (
    <div className="space-y-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl md:text-5xl font-display font-black text-white tracking-tighter mb-2">Payments</h1>
          <p className="text-gray-500 font-black text-[10px] uppercase tracking-[0.2em]">Monitor and manage all financial transactions</p>
        </div>
        <div className="flex items-center space-x-4">
          <div className="px-6 py-3 bg-brand-card rounded-2xl border border-white/5 shadow-xl text-[10px] font-black text-gray-500 uppercase tracking-widest">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-white/5">
              {filteredPayments.map((pay, idx) => (
                ...
                >
                  <td className="py-6 px-4 text-xs font-black text-white group-hover:text-brand-purple transition-colors">#{pay.id.slice(-6).toUpperCase()}</td>
                  <td className="py-6 px-4 text-xs font-bold text-gray-500">{pay.userEmail}</td>
                  <td className="py-6 px-4 text-xs font-black text-white">UGX {pay.amount?.toLocaleString()}</td>
                  <td className="py-6 px-4 text-xs font-bold text-gray-400">{pay.method}</td>
                  <td className="py-6 px-4 text-xs font-mono text-gray-600">{pay.phoneNumber || 'N/A'}</td>
                  ...
                  <td className="py-6 px-4 text-[10px] font-bold text-gray-600">{new Date(pay.createdAt).toLocaleDateString()}</td>
                  <td className="py-6 px-4 text-right">
                    <div className="flex items-center justify-end space-x-2">
                      {pay.status === 'Pending' && (
                        <>
                          <button 
                            ...
                            className="w-10 h-10 rounded-xl bg-emerald-500/10 text-emerald-500 hover:bg-emerald-50 hover:text-white transition-all flex items-center justify-center border border-emerald-500/20"
                      ...
                      <button className="w-10 h-10 rounded-xl bg-white/5 text-gray-500 hover:text-brand-purple hover:bg-white/10 transition-all flex items-center justify-center border border-white/5">
          ...
          {filteredPayments.length === 0 && !isLoading && (
            <div className="py-32 text-center space-y-4">
              ...
              <div className="text-gray-600 font-black uppercase tracking-widest text-xs">No payment requests found</div>
            </div>
          )}
        </div>
      </div>
