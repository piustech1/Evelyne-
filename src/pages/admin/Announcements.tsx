  ...
  return (
    <div className="space-y-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl md:text-5xl font-display font-black text-white tracking-tighter mb-2">Announcements</h1>
          <p className="text-gray-500 font-black text-[10px] uppercase tracking-[0.2em]">Post updates and news for your users</p>
        </div>
        <button 
          onClick={() => {
            setEditingAnn(null);
      ...
      <div className="grid grid-cols-1 gap-8">
        {announcements.map((ann, idx) => (
          <motion.div
            ...
            className="bg-brand-card p-8 md:p-10 rounded-[3rem] shadow-2xl border border-white/5 flex flex-col md:flex-row items-start justify-between group gap-6"
          >
            <div className="flex items-start space-x-6">
              ...
              <div className="space-y-3">
                <div className="flex items-center space-x-4">
                  <h3 className="text-2xl font-display font-black text-white tracking-tighter">{ann.title}</h3>
                  <span className={`px-4 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border ${
                    ann.status === 'Active' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-white/5 text-gray-600 border-white/5'
                  }`}>
                    {ann.status}
                  </span>
                </div>
                <p className="text-gray-500 font-medium text-sm max-w-2xl leading-relaxed">{ann.content}</p>
                <div className="flex items-center space-x-2 text-[10px] font-black text-gray-600 uppercase tracking-[0.2em] pt-2">
            ...
            <div className="flex items-center space-x-3">
              <button 
                ...
                className="w-12 h-12 rounded-2xl bg-white/5 text-gray-500 hover:text-brand-blue hover:bg-brand-blue/10 transition-all flex items-center justify-center border border-white/5"
              >
                <FontAwesomeIcon icon={faEdit} />
              </button>
              <button 
                onClick={() => handleDelete(ann.id)}
                className="w-12 h-12 rounded-2xl bg-white/5 text-gray-500 hover:text-rose-500 hover:bg-rose-500/10 transition-all flex items-center justify-center border border-white/5"
              >
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="space-y-3">
                ...
                <textarea
                  ...
                  className="w-full p-5 bg-white/5 border border-white/10 rounded-2xl text-white focus:outline-none focus:ring-4 focus:ring-brand-purple/10 focus:border-brand-purple transition-all font-bold h-40"
              ...
              <div className="space-y-3">
                ...
                >
                  ...
                  <option value="Expired" className="bg-brand-dark">Expired</option>
                </select>
              </div>
